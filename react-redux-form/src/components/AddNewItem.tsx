...
import { deleteListItemData as deleteListItemAction,selectListSectionItems } from "../store/formSlice";
...
const AddNewItem: React.FC<Props> = ({ sectionName }) => {
  ...
  selectListSectionItems(state, sectionName));
  console.log(`[AddNewItem - ${sectionName}] Rendering. Items:`, sectionItems);
  ...
  const handleOpenModalToEdit = (index: number) => {  
    const itemToEdit = sectionItems[index];
    if (itemToEdit) {
        setEditIndex(index);
...
const handleDeleteItem = (index: number) => {
  dispatch(deleteListItemAction({ section: sectionName, index }));
};
